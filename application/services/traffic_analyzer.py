...
class TrafficAnalyzerService:
    ...
    def analyze_video(
        ...
        while True:
            ...
            if frame_callback:
                # Pass ROI to callback if it accepts it
                try:
                    should_stop = frame_callback(frame, vehicles, frame_stats, frame_number, roi=self.roi)
                except TypeError:
                    # Backward compatibility: if callback doesn't accept roi parameter
